...
if [[ "$BUILD_ENVIRONMENT" == *-bazel-* ]]; then
  ...
  if [[ "$BUILD_ENVIRONMENT" != *libtorch* ]]; then
    ...
    if [[ "$BUILD_ENVIRONMENT" != *rocm*  && "$BUILD_ENVIRONMENT" != *xla* && "$BUILD_ENVIRONMENT" != *riscv64* && "$BUILD_ENVIRONMENT" != *ppc64le* ]]; then
...
# don't do this for bazel or s390x or ppc64le or riscv64 as they don't use sccache
